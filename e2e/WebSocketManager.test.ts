...
describe('WebSocketManager E2E Tests', () => {
  ...
  it('should send a command and receive a response', async () => {
    ...
    const response = await wsManager.sendCommand({ command: 'ping', arguments: {} });

    expect(sendCommandSpy).toHaveBeenCalledWith({ command: 'ping', arguments: {} });
  ...
  it('should disconnect successfully', async () => {
    ...
    expect(sendCommandSpy).toHaveBeenCalledWith({ command: 'logout', arguments: {} });
